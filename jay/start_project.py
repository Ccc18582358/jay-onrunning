    ...
    def run(self):
        ...
        for entity in entities:
            ...
            with open("%ss/%s_%s.py" % (
                    entity, class_name.lower(), entity), "w", encoding='utf-8') as f:
                f.write(
                    template(entity + ".py.tmpl",
                             ...
                             scu_xpath=(
                                 self.args.scu_xpath, '"'
                                 if self.args.scu_xpath.count("'") else "'"),
                             bucket_xpath=(
                                 self.args.bucket_xpath, '"'
                                 if self.args.bucket_xpath.count("'") else "'"),
                             )
                )
        print(", ".join("%s%s" % (class_name, entity.capitalize())
                        for entity in entities) + " have been created.")

    def enrich_parser_argument(self, parser):
        parser.add_argument("-s", "--spider", required=True, help="Spider name")
        parser.add_argument("-ix", "--item-xpath", default="",
                            help="Spider item xpath:"
                                 " xpath expression for acquire item link. ")
        parser.add_argument("-px", "--page-xpath", default="",
                            help="Spider page xpath: "
