...
from jay.itemloaders import ItemLoader
from scrapy.utils.misc import arg_to_iter
from jay.itemloaders.processors import Compose, MapCompose
...
def url_path_arg_increment(pattern_str, url):
    ...
    'http://www.backcountry.com.hk/en/men-apparel-shirts/page/2/'
    其中http://www.backcountry.com.hk/en/men-apparel-shirts为原url
    /page/为第一组所匹配; 2为第二组匹配的; /为第三组匹配的
    当给出的url为'http://www.backcountry.com.hk/en/men-apparel-shirts/page/2/'
    输出结果为'http://www.backcountry.com.hk/en/men-apparel-shirts/page/3/'
    @param pattern_str: r'1~=(/page/)(\d+)(/)'
    @param url: 'http://www.backcountry.com.hk/en/men-apparel-shirts‘
    @return:'http://www.backcountry.com.hk/en/men-apparel-shirts/page/2/'
    """
    parts = urlparse(url)
...
class ItemCollector(object):
    ...
    def trace(self, tracepoint):
        self.trace_log.append(tracepoint)

    def set_trace_id(self, id):
        if self.trace_id:
            raise "reset trace_id, original: {}, new: {}".format(self.trace_id, id)
        self.trace_id = id
    ...
    def pop_item_loader(self, path):
        return self.item_loaders.pop(path, None)
...
def extract_specs(sel, xpath, child_xpath="th//text()|td//text()"):
    """
    :param sel: selector
    :param xpath: xpath for each entry of k_v pair
    :param child_xpath: a string to extract k_v pair, or a tuple contains a key xpath and a value xpath
    :return: specs
    """
    specs = []
    for tr in sel.xpath(xpath):
        if tuple == type(child_xpath):
            k_v = tuple(' '.join(x.strip() for x in tr.xpath(xpath).extract()).strip() for xpath in child_xpath)
        else:  # str == type(child_xpath)
            k_v = tuple(x.strip() for x in tr.xpath(child_xpath).extract())
        if len(k_v) == 2 and any(k_v):
            specs.append(k_v)
    return specs
...
def findCaller(stack_info=False):
    ...
    while hasattr(f, "f_code"):
        ...
        if not (filename.count("jay") and not filename.count("utils")):
            f = f.f_back
            continue
        sinfo = None
        if stack_info:
...
def dump2file(file_name, content_in_utf8, folder_name='.'):
    """debug method

    一般断点时查看数据内容 self.dump2file('D:/1.json', response.unicode_as_body())
    """
...
def response_to_str(response):
    return "{status} {url}, body_size: {body_size}".format(
        status=response.status, url=response.url, headers=response.headers,
        body_size=len(response.body))


if __name__ == "__main__":
    print(url_path_arg_increment(r"0~=(/)(\d+)()",
